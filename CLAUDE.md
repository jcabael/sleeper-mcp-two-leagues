...
7. **Diagnose from live calls, never from tool descriptions.** An MCP tool's name/docstring (including default values it mentions, e.g. a generic "defaults to league X" blurb) is static documentation, not evidence of what it's actually returning right now. Never conclude a server is "misconfigured" or "bound to the wrong league" from reading its tool description via `ToolSearch` or similar — that text may be boilerplate left over from a template and can be wrong or stale. Before flagging a data-access problem, actually call the tool and inspect the real response (e.g. check the `league_id`/`name` field `get_league_info` returns). Only a live call's actual output is valid evidence of a misconfiguration. If a live call fails, that's when rule 4's retry-then-fallback applies — not before.
